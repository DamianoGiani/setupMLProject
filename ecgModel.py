from tensorflow.keras.layers import (
    Input, Conv1D, LSTM, MaxPooling1D, Dropout, BatchNormalization, Activation, Add, Flatten, Dense, Dropout)
...
def get_lstm_model(n_classes, last_layer='sigmoid'):
    kernel_initializer = 'he_normal'
    signal = Input(shape=(4096, 12), dtype=np.float32, name='signal')
    x = signal
    lstm = LSTM(128, return_sequences=True)(x)
    drop = Dropout(0.25)(lstm)
    # maybe add layer normalization before activation?
    lstm = LSTM(128, return_sequences=True)(drop)
    drop = Dropout(0.25)(lstm)
    x = Flatten()(drop)
    diagn = Dense(n_classes, activation=last_layer, kernel_initializer=kernel_initializer)(x)
    return Model(signal, diagn)
