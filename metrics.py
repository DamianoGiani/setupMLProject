...
def get_optimal_precision_recall(y_true, y_score):
    ...
    for k in range(n):
        ...
        f1_score = np.nan_to_num(2 * precision * recall / (precision + recall + 0.0001))
...
if __name__ == '__main__':
    ...
    # y_score_val = np.load('./outputs/dnn_output_val.npy')
    ...
    # _, _, threshold = get_optimal_precision_recall(y_true, y_score_val)
    # threshold = [round(elem, 3) for elem in threshold]
    # print(threshold)

    # original thresholds
    # threshold = np.array([0.124, 0.07, 0.05, 0.278, 0.390, 0.174])
    np.savetxt('./outputs/results/tables/thresholdAndPrecision.txt', np.append(threshold, micro_avg_precision), fmt='%f')
    ...
    # %% Generate table with scores for the model (table 2)
    scores_list = []
